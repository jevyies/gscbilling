...
$filename = 'GSCCONSTRUCTIONBILLING-'. date('mdY') .'.xls';
...
foreach($records as $record){
    ...
    $Confirmed_by_desig = $record->Confirmed_by_desig;
    $Noted_by_desig = $record->Noted_by_desig;
    $SOADate = date('F j, Y', strtotime($record->Date));
    break;
}
?>
    ...
    <body>
        ...
        <table width="100%">
            <tr>
                ...
                <td style="width:45%;">SOA #: <?php echo $SOANo; ?><br><br>DATE: <?php echo $SOADate; ?></td>
            </tr>
        </table>
        <table width="100%" class="main_t">
            <thead>
                <tr>
                    ...
                    <td colspan="2">ACCOUNT TO CHARGE</td>
                    <td colspan="28" style="text-align:center;">HOURS RENDERED</td>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align:right;"><?php echo number_format(($record->rd_st * $record->rd_st_r) + ($record->shol_st * $record->shol_st_r) + ($record->rhol_st * $record->rhol_st_r) + ($record->shrd_st * $record->shrd_st_r) + ($record->rhrd_st * $record->rhrd_st_r) + ($record->rhol_st2x * $record->rhol_st2x_r) + ($record->rholrd_st2x * $record->rholrd_st2x_r), 2, '.', ','); ?></td>
                ...
                $total_st = $total_st + ($record->rd_st * $record->rd_st_r) + ($record->shol_st * $record->shol_st_r) + ($record->rhol_st * $record->rhol_st_r) + ($record->shrd_st * $record->shrd_st_r) + ($record->rhrd_st * $record->rhrd_st_r) + ($record->rhol_st2x * $record->rhol_st2x_r) + ($record->rholrd_st2x * $record->rholrd_st2x_r);
        ...
        <table width="100%">
				...
				<tr>
					<td style="width:40%;"><br><br><b><u><?php echo strtoupper($Prepared_by).'/'.strtoupper($Checked_by); ?></u></b><br>GSMPC-BILLING CLERK</td>
					<td></td>
					<td style="width:40%;"></td>
				...
				<tr>
					<td style="width:40%;"><br><br><b><u><?php echo strtoupper($Confirmed_by); ?></u></b><br><?php echo strtoupper($Confirmed_by_desig) ?></td>
					<td></td>
