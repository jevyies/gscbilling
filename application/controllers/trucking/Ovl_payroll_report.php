...
class OVL_Payroll_Report extends REST_Controller
{
    ...
    public function index_get()
    {
        ...
        $records = array(
            'records' => $this->ovl_report_model->get_payroll_report($data),
            'from' => $this->get('from') ? $this->get('from') : '',
            'to' => $this->get('to') ? $this->get('to') : '',
        ...
        if($this->get('excel')){
            $html = $this->load->view('OVL_Payroll_Report', $records);
        }else{
            $html = $this->load->view('OVL_Payroll_Report', $records, true);
        }
        $mpdf = new \Mpdf\Mpdf([
            'default_font_size' => 9,
            'default_font' => 'tahoma',
