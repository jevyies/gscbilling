...
class Jeep_Standard_Report extends REST_Controller
{
    ...
    public function index_get()
    {
        $data = array(
            ...
            'vID' => $this->get('vID') ? $this->get('vID') : '',
        );
        $records = array(
            'records' => $this->jeep_report_model->get_standard_report($data),
        ...
        if($this->get('excel')){
            $html = $this->load->view('Jeep_Standard_Report', $records);
        }else{
            $html = $this->load->view('Jeep_Standard_Report', $records, true);
        }
        $mpdf = new \Mpdf\Mpdf([
            'default_font_size' => 9,
            'default_font' => 'tahoma',
