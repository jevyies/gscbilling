...
class OC_BCC extends REST_Controller {
    ...
    public function index_get(){
        if($this->get('getHeader')){
            $result = $this->oc_bcc_model->get_header();
        }elseif($this->get('getHeaderID')){
            $result = $this->oc_bcc_model->get_transmittal_details($this->get('getHeaderID'));
        }elseif($this->get('getTotalBilling')){
            $result = $this->oc_bcc_model->get_total_billing($this->get('hdr_id'));
        }elseif($this->get('getDtl')){
            $result = $this->oc_bcc_model->get_detail($this->get('hdr_id'));
        }elseif($this->get('getRates')){
            $result = $this->oc_bcc_model->get_rates($this->get('activity_id'));
        }elseif($this->get('getRatesSelected')){
            $result = $this->oc_bcc_model->get_rates_selected($this->get('hdr_id'));
        }elseif($this->get('getReportPreviewData')){
            $result = $this->oc_bcc_model->get_report_preview($this->get('id'));
        }elseif($this->get('generateReport')){
            $records = array(
                'records' => $this->oc_bcc_model->get_report_preview($this->get('id')),
            );
            // $html = $this->load->view('DEARBC_report', $records);
            $html = $this->load->view('BCC_Letterhead', $records, true);
            $mpdf = new \Mpdf\Mpdf([
                'default_font_size' => 9,
                'default_font' => 'tahoma',
            ]); 
            ...
            $records = array(
                'records' => $this->oc_bcc_model->get_report($this->get('id')),
            );
            $html = $this->load->view('BCC_report', $records);
            // $html = $this->load->view('BCC_report', $records, true);
            $mpdf = new \Mpdf\Mpdf([
                'default_font_size' => 9,
                'default_font' => 'tahoma',
            ]); 
    ...
    public function index_post(){
        if($this->post('save_header')){
            ...
            $result = $this->oc_bcc_model->save_header($data);
            ...
            $result = $this->oc_bcc_model->save_detail($data);
            if(gettype($result) ===  "array"){
                $result = array(
                    'success' => true,
                    'id' => $result['id'],
            ...
            $result = $this->oc_bcc_model->transmit_data($this->post('id'), $data);
            if($result){
                $result = array(
                    'success' => true,
            ...
            $result = $this->oc_bcc_model->reactivate_data($data);
            if($result){
                $result = array(
                    'success' => true,
            ...
            $result = $this->oc_bcc_model->save_print_preview($data);
            if($result){
                $result = array(
                    'success' => true,
        ...
        }elseif($this->post('getAccess')){
            $data = array(
                'email' => $this->post('email') ? $this->post('email') : '',
                'password' => $this->post('password') ? $this->post('password') : '',
            );
            $result = $this->oc_bcc_model->get_access($data);
            if($result){
                $result = array(
                    'success' => true,
                    'message' => 'Successfully saved',
                    'reactivated_by' => $result
                );
                $this->response($result, REST_Controller::HTTP_OK);
            }else{
                $result = array(
    ...
    public function index_delete(){
        $result = $this->oc_bcc_model->delete_header($this->query('id'));
        if ($result){
            $result = array(
                'success' => true,
    ...
    public function detail_delete(){
        $result = $this->oc_bcc_model->delete_detail($this->query('id'));
        if ($result){
            $result = array(
                'success' => true,
                'message' => 'Successfully deleted'
