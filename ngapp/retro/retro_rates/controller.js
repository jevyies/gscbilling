angular.module('app')
    .controller('RetroRateCtrl', RetroRateCtrl)
    .controller('RetroDARRateCtrl', RetroDARRateCtrl)
    .controller('RetroUploadDARRateCtrl', RetroUploadDARRateCtrl)
    .controller('RetroSARRateCtrl', RetroSARRateCtrl)
    .controller('RetroBCCRateCtrl', RetroBCCRateCtrl)
    .controller('RetroSlersRateCtrl', RetroSlersRateCtrl)
    .controller('RetroClubHouseRateCtrl', RetroClubHouseRateCtrl)
    .controller('RetroReactivateRateCtrl', RetroReactivateRateCtrl)
    .controller('RetroUploadSLERSRateCtrl', RetroUploadSLERSRateCtrl)
    .controller('RetroUploadBCCRateCtrl', RetroUploadBCCRateCtrl)
    .controller('RetroUploadCLUBHOUSERateCtrl', RetroUploadCLUBHOUSERateCtrl)

RetroRateCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector'];
RetroDARRateCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
RetroUploadDARRateCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance', '$q'];
RetroSARRateCtrl = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
RetroBCCRateCtrl = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
RetroSlersRateCtrl = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
RetroClubHouseRateCtrl = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
RetroReactivateRateCtrl = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
RetroUploadSLERSRateCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance', '$q'];
RetroUploadBCCRateCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance', '$q'];
RetroUploadCLUBHOUSERateCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance', '$q'];

function RetroRateCtrl($scope, $ocLazyLoad, $injector) {
    var vm = this;
    vm.data = [];
    vm.list = [];
    vm.listSAR = [];
    ...
    vm.type = 'DAR';
    $ocLazyLoad.load([
        RETROURL + 'retro_rates/service.js?v=' + VERSION,
    ]).then(function (d) {
        RetroRateSvc = $injector.get('RetroRateSvc');
        vm.getRates()
    });
    ...
    vm.getRates = function () {
        ...
        RetroRateSvc.get().then(function (response) {
            if (response.message) {
                vm.data = [];
            } else {
                vm.data = response;
    ...
    vm.getSARRates = function () {
        LOADING.classList.add("open");
        RetroRateSvc.get({ sar: true }).then(function (response) {
            if (response.message) {
                vm.listSAR = [];
            } else {
                vm.listSAR = response;
    ...
    vm.getBCCRates = function () {
        LOADING.classList.add("open");
        RetroRateSvc.get({ bcc: true }).then(function (response) {
            if (response.message) {
                vm.listBCC = [];
            } else {
                vm.listBCC = response;
    ...
    vm.getSlersRates = function () {
        LOADING.classList.add("open");
        RetroRateSvc.get({ slers: true }).then(function (response) {
            if (response.message) {
                vm.listSlers = [];
            } else {
    ...
    vm.getClubhouseRates = function () {
        LOADING.classList.add("open");
        RetroRateSvc.get({ clubhouse: true }).then(function (response) {
            if (response.message) {
                vm.listClubhouse = [];
            } else {
    ...
    vm.defaultGrid = {
        ...
        columnDefs: [
            ...
            { name: "Uploaded By", field: "UploadedBy", width: 120 },
            { name: "Upload Date", field: "created_at", width: 120 },
        ],
        data: "vm.list",
    ...
    vm.defaultGridSlers = {
        ...
        enableRowHeaderSelection: false,
        columnDefs: [
            { name: "Description", field: "date_uploaded_description", displayName: 'Description', width: 300, hidePinLeft: false, },
            { name: "gl", field: "gl_fr_mg", displayName: 'gl', width: 140, hidePinLeft: false, },
            { name: "Activity", field: "activity_fr_mg", width: 300, pinnedLeft: true },
            { name: "costcenter", field: "costcenter__", displayName: 'costcenter', width: 140 },
            { name: "status", field: "status", displayName: 'status', width: 120 },
    ...
    vm.DarRate = function (row) {
        ...
        var options = {
            data: data,
            animation: true,
            templateUrl: RETROURL + "retro_rates/add_rate_dar.html?v=" + VERSION,
            controllerName: "RetroDARRateCtrl",
            viewSize: "lg",
            filesToLoad: [
                RETROURL + "retro_rates/add_rate_dar.html?v=" + VERSION,
                RETROURL + "retro_rates/controller.js?v=" + VERSION
            ]
        };
    ...
    vm.SarRate = function (row) {
        ...
        var options = {
            data: data,
            animation: true,
            templateUrl: RETROURL + "retro_rates/add_rate_sar.html?v=" + VERSION,
            controllerName: "RetroSARRateCtrl",
            viewSize: "lg",
            filesToLoad: [
                RETROURL + "retro_rates/add_rate_sar.html?v=" + VERSION,
                RETROURL + "retro_rates/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options).then(function (data) {
    ...
    vm.BCCRate = function (row) {
        ...
        var options = {
            data: data,
            animation: true,
            templateUrl: RETROURL + "retro_rates/add_rate_bcc.html?v=" + VERSION,
            controllerName: "RetroBCCRateCtrl",
            viewSize: "lg",
            filesToLoad: [
                RETROURL + "retro_rates/add_rate_bcc.html?v=" + VERSION,
                RETROURL + "retro_rates/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options).then(function (data) {
            if (data) {
                if (row) {
                    vm.listBCC.splice(row.index, 1, data);
                } else {
                    vm.listBCC.push(data);
                }
            }
        });
    }
    vm.SlersRate = function (row) {
        ...
        var options = {
            data: data,
            animation: true,
            templateUrl: RETROURL + "retro_rates/add_rate_slers.html?v=" + VERSION,
            controllerName: "RetroSlersRateCtrl",
            viewSize: "lg",
            filesToLoad: [
                RETROURL + "retro_rates/add_rate_slers.html?v=" + VERSION,
                RETROURL + "retro_rates/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options).then(function (data) {
            if (data) {
                if (row) {
                    vm.listSlers.splice(row.index, 1, data);
                } else {
                    vm.listSlers.push(data);
                }
            }
        });
    }
    vm.ClubhouseRate = function (row) {
        ...
        var options = {
            data: data,
            animation: true,
            templateUrl: RETROURL + "retro_rates/add_rate_clubhouse.html?v=" + VERSION,
            controllerName: "RetroClubHouseRateCtrl",
            viewSize: "lg",
            filesToLoad: [
                RETROURL + "retro_rates/add_rate_clubhouse.html?v=" + VERSION,
                RETROURL + "retro_rates/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options).then(function (data) {
            if (data) {
                if (row) {
                    vm.listClubhouse.splice(row.index, 1, data);
                } else {
                    vm.listClubhouse.push(data);
                }
            }
        });
    }
    vm.reactivateRate = function () {
        var options = {
            data: 'menu',
            animation: true,
            templateUrl: RETROURL + "retro_rates/reactivate_rate.html?v=" + VERSION,
            controllerName: "RetroReactivateRateCtrl",
            viewSize: "md",
            filesToLoad: [
                RETROURL + "retro_rates/reactivate_rate.html?v=" + VERSION,
                RETROURL + "retro_rates/controller.js?v=" + VERSION
            ]
        };
...
function RetroDARRateCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.variables = {};
    if (data.id) {
    ...
    modal.save = function () {
        ...
        LOADING.classList.add("open");
        RetroRateSvc.save(modal.variables).then(function (response) {
            if (response.success) {
                if (response.id) {
    ...
    modal.upload = function () {
        var options = {
            data: 'menu',
            animation: true,
            templateUrl: RETROURL + "retro_rates/upload_excel_dar.html?v=" + VERSION,
            controllerName: "RetroUploadDARRateCtrl",
            viewSize: "md",
            filesToLoad: [
                RETROURL + "retro_rates/upload_excel_dar.html?v=" + VERSION,
                RETROURL + "retro_rates/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options);
    ...
    modal.delete = function () {
        AppSvc.confirmation('Confirm Deletion?', 'Are you sure you want to delete this record?', 'Delete', 'Cancel', true).then(function (data) {
            if (data) {
                RetroRateSvc.save({ deleteDAR: true, id: modal.variables.id }).then(function (response) {
                    if (response.success) {
...
function RetroUploadDARRateCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance, $q) {
    var modal = this;
    modal.list = [];
    $ocLazyLoad.load([
        RETROURL + 'retro_rates/service.js?v=' + VERSION,
    ...
    modal.uploadExcel = function (start, end) {
        ...
        if (totalArray < 100) {
            ...
            RetroRateSvc.save({ uploadDAR: true, list: list }).then(function (response) {
                if (response.success) {
    ...
    modal.finalizeUpload = function () {
        RetroRateSvc.save({ finalizeDAR: true }).then(function (response) {
            if (response.success) {
                $uibModalInstance.dismiss('cancel');
                LOADING.classList.remove("open");
...
function RetroSARRateCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.variables = {};
...
function RetroBCCRateCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.variables = {};
    modal.variables.status = 'active';
    if (data.id) {
    ...
    modal.upload = function () {
        var options = {
            data: 'menu',
            animation: true,
            templateUrl: RETROURL + "retro_rates/upload_excel_bcc.html?v=" + VERSION,
            controllerName: "RetroUploadBCCRateCtrl",
            viewSize: "md",
            filesToLoad: [
                RETROURL + "retro_rates/upload_excel_bcc.html?v=" + VERSION,
                RETROURL + "retro_rates/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options);
...
function RetroSlersRateCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.variables = {};
    modal.variables.status = 'active';
    if (data.id) {
    ...
    modal.upload = function () {
        var options = {
            data: 'menu',
            animation: true,
            templateUrl: RETROURL + "retro_rates/upload_excel_slers.html?v=" + VERSION,
            controllerName: "RetroUploadSLERSRateCtrl",
            viewSize: "md",
            filesToLoad: [
                RETROURL + "retro_rates/upload_excel_slers.html?v=" + VERSION,
                RETROURL + "retro_rates/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options);
...
function RetroClubHouseRateCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.variables = {};
    modal.variables.status = 'active';
    if (data.id) {
    ...
    modal.upload = function () {
        var options = {
            data: 'menu',
            animation: true,
            templateUrl: RETROURL + "retro_rates/upload_excel_clubhouse.html?v=" + VERSION,
            controllerName: "RetroUploadCLUBHOUSERateCtrl",
            viewSize: "md",
            filesToLoad: [
                RETROURL + "retro_rates/upload_excel_clubhouse.html?v=" + VERSION,
                RETROURL + "retro_rates/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options);
...
function RetroReactivateRateCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    ...
    modal.displayUploads = function () {
        RetroRateSvc.get({ displayUploads: true }).then(function (response) {
            if (response.message) {
                modal.display = [];
            } else {
    ...
    modal.reactivate = function () {
        ...
        RetroRateSvc.save({ reactivateRate: true, id: modal.variables.id }).then(function (response) {
            if (response.success) {
                $uibModalInstance.close(modal.variables);
                AppSvc.showSwal('Success', response.message, 'success');
            } else {
...
function RetroUploadBCCRateCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance, $q) {
    var modal = this;
    modal.list = [];
    $ocLazyLoad.load([
        RETROURL + 'retro_rates/service.js?v=' + VERSION,
    ...
    modal.uploadExcel = function (start, end) {
        ...
        if (totalArray < 100) {
            list = modal.list.slice(start, totalArray);
            RetroRateSvc.save({ uploadBCC: true, list: list }).then(function (response) {
                if (response.success) {
                    modal.finalizeUpload();
                } else {
                    LOADING.classList.remove("open");
    ...
    modal.finalizeUpload = function () {
        RetroRateSvc.save({ finalizeBCC: true }).then(function (response) {
            if (response.success) {
                $uibModalInstance.dismiss('cancel');
                LOADING.classList.remove("open");
                return AppSvc.showSwal('Success', response.message, 'success');
...
function RetroUploadSLERSRateCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance, $q) {
    var modal = this;
    modal.list = [];
    $ocLazyLoad.load([
        RETROURL + 'retro_rates/service.js?v=' + VERSION,
    ...
    modal.uploadExcel = function (start, end) {
        ...
        if (totalArray < 100) {
            list = modal.list.slice(start, totalArray);
            RetroRateSvc.save({ uploadSLERS: true, list: list }).then(function (response) {
                if (response.success) {
                    modal.finalizeUpload();
                } else {
                    LOADING.classList.remove("open");
    ...
    modal.finalizeUpload = function () {
        RetroRateSvc.save({ finalizeSLERS: true }).then(function (response) {
            if (response.success) {
                $uibModalInstance.dismiss('cancel');
                LOADING.classList.remove("open");
                return AppSvc.showSwal('Success', response.message, 'success');
...
function RetroUploadCLUBHOUSERateCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance, $q) {
    var modal = this;
    modal.list = [];
    $ocLazyLoad.load([
        RETROURL + 'retro_rates/service.js?v=' + VERSION,
    ...
    modal.uploadExcel = function (start, end) {
        ...
        if (totalArray < 100) {
            list = modal.list.slice(start, totalArray);
            RetroRateSvc.save({ uploadCLUBHOUSE: true, list: list }).then(function (response) {
                if (response.success) {
                    modal.finalizeUpload();
                } else {
                    LOADING.classList.remove("open");
    ...
    modal.finalizeUpload = function () {
        RetroRateSvc.save({ finalizeCLUBHOUSE: true }).then(function (response) {
            if (response.success) {
                $uibModalInstance.dismiss('cancel');
                LOADING.classList.remove("open");
                return AppSvc.showSwal('Success', response.message, 'success');
