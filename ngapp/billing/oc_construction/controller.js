angular.module('app')
    .controller('OCConstructionCTRL', OCConstructionCTRL)
    .controller('OCConstructionHDRCTRL', OCConstructionHDRCTRL)
    .controller('OCConstructionSearchHdrCtrl', OCConstructionSearchHdrCtrl)
    .controller('OCConstructionDtlCtrl', OCConstructionDtlCtrl)
    .controller('OCConstructionPrintPreviewCtrl', OCConstructionPrintPreviewCtrl)
    .controller('TransmitBillingCtrl', TransmitBillingCtrl)

OCConstructionCTRL.$inject = ['$scope', '$ocLazyLoad', '$injector'];
OCConstructionHDRCTRL.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
OCConstructionSearchHdrCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
OCConstructionDtlCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
OCConstructionPrintPreviewCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
TransmitBillingCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];

function OCConstructionCTRL($scope, $ocLazyLoad, $injector) {
    var vm = this;
    vm.header = {};
    vm.data = [];
    vm.list = [];
    ...
    $ocLazyLoad.load([
        BILLINGURL + 'oc_construction/service.js?v=' + VERSION,
    ]).then(function(d) {
        OCConstructionSvc = $injector.get('OCConstructionSvc');
    });
    ...
    vm.newHeader = function(pass_value) {
        ...
        var options = {
            data: data,
            animation: true,
            templateUrl: BILLINGURL + "oc_construction/new_header.html?v=" + VERSION,
            controllerName: "OCConstructionHDRCTRL",
            viewSize: "lg",
            filesToLoad: [
                BILLINGURL + "oc_construction/new_header.html?v=" + VERSION,
                BILLINGURL + "oc_construction/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options).then(function(data) {
            if (data) {
                ...
                vm.header.date_transmitted = new Date(data.date_transmitted);
                vm.header.Date = new Date(data.Date);
    ...
    vm.getTotalBilling = function(hdr_id) {
        ...
        OCConstructionSvc.get({ getTotalBilling: true, hdr_id: hdr_id }).then(function(response) {
            if (!response.message) {
    ...
    vm.getRatesSelected = function(hdr_id) {
        if (hdr_id) {
            LOADING.classList.add("open");
            OCConstructionSvc.get({ getRatesSelected: true, hdr_id: hdr_id }).then(function(response) {
                if (response.message) {
                    vm.dataRates = [];
                } else {
    ...
    vm.searchHeader = function() {
        var options = {
            data: 'menu',
            animation: true,
            templateUrl: BILLINGURL + "oc_construction/search_header.html?v=" + VERSION,
            controllerName: "OCConstructionSearchHdrCtrl",
            viewSize: "lg",
            filesToLoad: [
                BILLINGURL + "oc_construction/search_header.html?v=" + VERSION,
                BILLINGURL + "oc_construction/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options).then(function(data) {
    ...
    vm.searchDtl = function(hdr_id) {
        ...
        LOADING.classList.add("open");
        OCConstructionSvc.get({ getDtl: true, hdr_id: hdr_id }).then(function(response) {
            if (response.message) {
                vm.data = [];
            } else {
                vm.data = response;
    ...
    vm.addEmployee = function(row) {
        ...
        var options = {
            ...
            animation: true,
            templateUrl: BILLINGURL + "oc_construction/add_employee.html?v=" + VERSION,
            controllerName: "OCConstructionDtlCtrl",
            viewSize: "lg",
            filesToLoad: [
                BILLINGURL + "oc_construction/add_employee.html?v=" + VERSION,
                BILLINGURL + "oc_construction/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options).then(function(data) {
    ...
    vm.delete = function() {
        ...
        OCConstructionSvc.delete(vm.header.TOCSHDR).then(function(response) {
            if (response.success) {
                vm.clear();
                LOADING.classList.remove("open");
    ...
    vm.transmit = function() {
        ...
        var options = {
            ...
            animation: true,
            templateUrl: BILLINGURL + "oc_construction/transmit_billing.html?v=" + VERSION,
            controllerName: "TransmitBillingCtrl",
            viewSize: "md",
            filesToLoad: [
                BILLINGURL + "oc_construction/transmit_billing.html?v=" + VERSION,
                BILLINGURL + "oc_construction/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options).then(function(data){
                    ...
                    if(data){
                        ...
                        AppSvc.modal(options).then(function(data){
                            if(data){
                                var reason = data;
                                LOADING.classList.add("open");
                                OCConstructionSvc.save({ id: vm.header.TOCSHDR, reactivation: true, reactivatedBy: reactivated_by, reasonofreactivation: reason }).then(function(response) {
                                    if (response.success) {
    ...
    vm.printPreview = function() {
        ...
        var options = {
            ...
            animation: true,
            templateUrl: BILLINGURL + "oc_construction/print_preview.html?v=" + VERSION,
            controllerName: "OCConstructionPrintPreviewCtrl",
            viewSize: "sm",
            filesToLoad: [
                BILLINGURL + "oc_construction/print_preview.html?v=" + VERSION,
                BILLINGURL + "oc_construction/controller.js?v=" + VERSION
            ]
        };
...
function OCConstructionHDRCTRL($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.variables = {};
        ...
        AppSvc.modal(options).then(function(data) {
            if (data) {
                if (number == 1) {
                    ...
                    modal.variables.Checked_by_t = data.fullname;
                    modal.variables.Checked_by_desig_t = data.position;
                } else if (number == 6) {
                    modal.variables.Received_by_t = data.fullname;
                    modal.variables.Received_by_desig_t = data.position;
                } else if (number == 7) {
                    modal.variables.Approved_by_t = data.fullname;
                    modal.variables.Approved_by_desig_t = data.position;
                }
            }
        })
    }
    ...
    modal.save = function() {
        ...
        data.save_header = true;
        OCConstructionSvc.save(data).then(function(response) {
            if (response.success) {
                if (response.id) {
...
function OCConstructionSearchHdrCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.data = [];
    modal.list = [];
    var filter = $injector.get('$filter');
    ...
    modal.getData = function() {
        LOADING.classList.add("open");
        OCConstructionSvc.get({ getHeader: true }).then(function(response) {
            if (response.message) {
                modal.data = [];
            } else {
                modal.data = response;
    ...
    modal.defaultGrid = {
        ...
        columnDefs: [
            ...
            { name: "Transmittal #", field: "transmittal_no" },
...
function OCConstructionDtlCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    var filter = $injector.get("$filter");
    modal.variables = {};
    modal.rates = {};
    ...
    if (data.variables.TOSDTL) {
        ...
        LOADING.classList.add("open");
        OCConstructionSvc.get({ getRates: true, activity_id: modal.variables.activity_id }).then(function(response) {
            if (!response.message) {
    ...
    $ocLazyLoad.load([
        ...
        OCConstructionSvc = $injector.get('OCConstructionSvc');
        OCConstructionDtlSvc = $injector.get('OCConstructionDtlSvc');
    });
    ...
    modal.delete = function() {
        ...
        OCConstructionDtlSvc.delete(modal.variables.TOSDTL).then(function(response) {
            if (response.success) {
...
function OCConstructionPrintPreviewCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.variables = {};
    if (data) {
    ...
    console.log(modal.variables.TOCSHDR);
    $ocLazyLoad.load([
        BILLINGURL + 'oc_construction/service.js?v=' + VERSION,
    ]).then(function(d) {
        OCConstructionSvc = $injector.get('OCConstructionSvc');
    ...
    modal.getData = function() {
        LOADING.classList.add("open");
        OCConstructionSvc.get({ getReportPreviewData: true, id: modal.variables.TOCSHDR }).then(function(response) {
            if (!response.message) {
    ...
    modal.generate = function() {
        window.open('api/oc_construction?generateReportDetails=true&id=' + modal.variables.TOCSHDR);
    }

    modal.generateTransmittal = function() {
        window.open('api/oc_construction?generateReport=true&id=' + modal.variables.transmittal_no);
    }
...
function TransmitBillingCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    ...
    modal.getData = function (){
        OCConstructionSvc.get({ getHeaderID: modal.variables.id }).then(function(response) {
            if (!response.message) {
                modal.variables = response[0];
                modal.variables.date_transmitted = new Date(response[0].date_transmitted);
            }
