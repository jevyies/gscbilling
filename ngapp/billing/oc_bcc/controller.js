angular.module('app')
    .controller('OCBCCCTRL', OCBCCCTRL)
    .controller('OCBCCHDRCTRL', OCBCCHDRCTRL)
    .controller('OCBCCSearchHdrCtrl', OCBCCSearchHdrCtrl)
    .controller('OCBCCDtlCtrl', OCBCCDtlCtrl)
    .controller('OCBCCPrintPreviewCtrl', OCBCCPrintPreviewCtrl)
    .controller('AccessRequiredCtrl', AccessRequiredCtrl)
    .controller('TransmitBillingCtrl', TransmitBillingCtrl)

OCBCCCTRL.$inject = ['$scope', '$ocLazyLoad', '$injector'];
OCBCCHDRCTRL.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
OCBCCSearchHdrCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
OCBCCDtlCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
OCBCCPrintPreviewCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
AccessRequiredCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];
TransmitBillingCtrl.$inject = ['$scope', '$ocLazyLoad', '$injector', 'data', '$uibModalInstance'];

function OCBCCCTRL($scope, $ocLazyLoad, $injector) {
    var vm = this;
    vm.header = {};
    vm.data = [];
    vm.list = [];
    ...
    $ocLazyLoad.load([
        BILLINGURL + 'oc_bcc/service.js?v=' + VERSION,
    ]).then(function(d) {
        OCBccSvc = $injector.get('OCBccSvc');
    });
    ...
    vm.newHeader = function(pass_value) {
        ...
        var options = {
            data: data,
            animation: true,
            templateUrl: BILLINGURL + "oc_bcc/new_header.html?v=" + VERSION,
            controllerName: "OCBCCHDRCTRL",
            viewSize: "lg",
            filesToLoad: [
                BILLINGURL + "oc_bcc/new_header.html?v=" + VERSION,
    ...
    vm.getTotalBilling = function(hdr_id) {
        ...
        OCBccSvc.get({ getTotalBilling: true, hdr_id: hdr_id }).then(function(response) {
            if (!response.message) {
    ...
    vm.getRatesSelected = function(hdr_id) {
        if (hdr_id) {
            LOADING.classList.add("open");
            OCBccSvc.get({ getRatesSelected: true, hdr_id: hdr_id }).then(function(response) {
                if (response.message) {
                    vm.dataRates = [];
                } else {
    ...
    vm.searchHeader = function() {
        var options = {
            data: 'menu',
            animation: true,
            templateUrl: BILLINGURL + "oc_bcc/search_header.html?v=" + VERSION,
            controllerName: "OCBCCSearchHdrCtrl",
            viewSize: "lg",
            filesToLoad: [
                BILLINGURL + "oc_bcc/search_header.html?v=" + VERSION,
                BILLINGURL + "oc_bcc/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options).then(function(data) {
    ...
    vm.searchDtl = function(hdr_id) {
        ...
        LOADING.classList.add("open");
        OCBccSvc.get({ getDtl: true, hdr_id: hdr_id }).then(function(response) {
            if (response.message) {
                vm.data = [];
            } else {
                vm.data = response;
    ...
    vm.addEmployee = function(row) {
        ...
        if (vm.header.Status == 'TRANSMITTED') {
            return AppSvc.showSwal("Warning", "BCC was already transmitted. Cannot update.", "warning");
        }
        var data = 'menu';
        if (row) {
        ...
        var options = {
            ...
            animation: true,
            templateUrl: BILLINGURL + "oc_bcc/add_employee.html?v=" + VERSION,
            controllerName: "OCBCCDtlCtrl",
            viewSize: "lg",
            filesToLoad: [
                BILLINGURL + "oc_bcc/add_employee.html?v=" + VERSION,
                BILLINGURL + "oc_bcc/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options).then(function(data) {
    ...
    vm.delete = function() {
        ...
        OCBccSvc.delete(vm.header.TOCSHDR).then(function(response) {
            if (response.success) {
                vm.clear();
                LOADING.classList.remove("open");
    ...
    vm.transmit = function() {
        ...
        var options = {
            ...
            animation: true,
            templateUrl: BILLINGURL + "oc_bcc/transmit_billing.html?v=" + VERSION,
            controllerName: "TransmitBillingCtrl",
            viewSize: "md",
            filesToLoad: [
                BILLINGURL + "oc_bcc/transmit_billing.html?v=" + VERSION,
                BILLINGURL + "oc_bcc/controller.js?v=" + VERSION
            ]
        };
        AppSvc.modal(options).then(function(data){
                    ...
                    if(data){
                        ...
                        AppSvc.modal(options).then(function(data){
                            if(data){
                                var reason = data;
                                LOADING.classList.add("open");
                                OCBccSvc.save({ id: vm.header.TOCSHDR, reactivation: true, reactivatedBy: reactivated_by, reasonofreactivation: reason }).then(function(response) {
                                    if (response.success) {
    ...
    vm.printPreview = function() {
        ...
        var options = {
            ...
            animation: true,
            templateUrl: BILLINGURL + "oc_bcc/print_preview.html?v=" + VERSION,
            controllerName: "OCBCCPrintPreviewCtrl",
            viewSize: "lg",
            filesToLoad: [
                BILLINGURL + "oc_bcc/print_preview.html?v=" + VERSION,
                BILLINGURL + "oc_bcc/controller.js?v=" + VERSION
            ]
        };
...
function OCBCCHDRCTRL($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.variables = {};
    ...
    modal.save = function() {
        ...
        OCBccSvc.save(modal.variables).then(function(response) {
            if (response.success) {
                if (response.id) {
...
function OCBCCSearchHdrCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.data = [];
    modal.list = [];
    var filter = $injector.get('$filter');
    ...
    modal.getData = function() {
        LOADING.classList.add("open");
        OCBccSvc.get({ getHeader: true }).then(function(response) {
            if (response.message) {
                modal.data = [];
            } else {
                modal.data = response;
...
function OCBCCDtlCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    var filter = $injector.get("$filter");
    modal.variables = {};
    modal.rates = {};
    ...
    if (data.variables.TOSDTL) {
        ...
        LOADING.classList.add("open");
        OCBccSvc.get({ getRates: true, activity_id: modal.variables.activity_id }).then(function(response) {
            if (!response.message) {
    ...
    $ocLazyLoad.load([
        ...
        OCBccSvc = $injector.get('OCBccSvc');
        OCBccDtlSvc = $injector.get('OCBccDtlSvc');
    });
    ...
    modal.searchActivity = function(client) {
        ...
        AppSvc.modal(options).then(function(data) {
            if (data) {
                ...
                OCBccSvc.get({ getRates: true, activity_id: data.id }).then(function(response) {
                    if (!response.message) {
                        modal.rates = angular.copy(response[0]);
                    } else {
                        modal.rates = {};
    ...
    modal.save = function() {
        ...
        OCBccSvc.save(data).then(function(response) {
            if (response.success) {
                if (response.id) {
    ...
    modal.delete = function() {
        ...
        OCBccDtlSvc.delete(modal.variables.TOSDTL).then(function(response) {
            if (response.success) {
...
function OCBCCPrintPreviewCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.variables = {};
    if (data) {
    ...
    modal.getData = function() {
        LOADING.classList.add("open");
        OCBccSvc.get({ getReportPreviewData: true, id: modal.variables.TOCSHDR }).then(function(response) {
            if (!response.message) {
                modal.variables = response[0];
                if (modal.variables.letter_to == '') {
                    modal.variables.letter_to = "<b>MS. Ginalou T. Cahatian</b><br>Admin/Finance Manage<br>BCC<br>Phillips, Manolo Fortich, Bukidnon";
                }
                if (modal.variables.body == '') {
                    modal.variables.body = "Dear Ms. Cahatian,<br><br>Attached are the supporting documents of out billing rendered by <b>GENERAL SERVICES MULTIPURPOSE COOPERATIVE</b> for the period <u>" + modal.variables.period_date + "</u>";
                }
    ...
    modal.generate = function() {
        ...
        OCBccSvc.save(modal.variables).then(function(response) {
            if (response.success) {
                window.open('api/oc_bcc?generateReport=true&id=' + modal.variables.TOCSHDR);
                window.open('api/oc_bcc?generateReportDetails=true&id=' + modal.variables.TOCSHDR);
            } else {
...
function AccessRequiredCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    var modal = this;
    modal.email = '';
    modal.password = '';
    modal.reason = '';
    ...
    modal.access = function() {
        LOADING.classList.add("open");
        OCBccSvc.save({ getAccess: true, email: modal.email, password: modal.password }).then(function(response) {
            if (response.success) {
                $uibModalInstance.close(response.reactivated_by);
            } else {
                AppSvc.showSwal("Sorry", "Invalid Access.", "warning");
            }
            LOADING.classList.remove("open");
        });
    }
    ...
    modal.reactivate = function() {
        if(!modal.reason){
            return AppSvc.showSwal("Requirement", "Please enter a reason for reactivation.", "warning");
        }
        $uibModalInstance.close(modal.reason);
    }
...
function TransmitBillingCtrl($scope, $ocLazyLoad, $injector, data, $uibModalInstance) {
    ...
    modal.getData = function (){
        OCBccSvc.get({ getHeaderID: modal.variables.id }).then(function(response) {
            if (!response.message) {
                modal.variables = response[0];
